...
function parseTicket(id: string) {
  const match = id.trim().toUpperCase().match(/^WC26-[A-Z]{2}-(\d{6})-([A-Z])(\d+)$/);
  if (!match) return null;
  return { section: Math.min(parseInt(match[1].slice(0, 3), 10) % 200, 199) + 100, row: match[2], seat: parseInt(match[3], 10) };
}
...
export default function TicketPanel() {
  ...
  const handleSubmit = () => { const p = parseTicket(ticketInput); if (p) { setTicket({ id: ticketInput.trim().toUpperCase(), ...p }); setError(''); } else { setError('Invalid format. Use: WC26-MX-XXXXXX-X00'); } };
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
        <GlassCard padding="p-3.5">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSubmit} className="px-3 py-2 bg-cyan-500/10 border border-cyan-500/18 rounded-xl text-cyan-400 hover:bg-cyan-500/18 transition-all"><Search className="w-3.5 h-3.5" /></button>
          </div>
        ...
        {ticket && (<>
          ...
          <button className="w-full bg-cyan-500/10 hover:bg-cyan-500/18 text-cyan-300 text-[12px] font-semibold py-2.5 rounded-xl transition-all border border-cyan-500/18 flex items-center justify-center gap-2">
