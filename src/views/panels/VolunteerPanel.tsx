  ...
  return (
    <div className="w-[320px] xl:w-[380px] h-full flex flex-col z-40 hidden md:flex relative" style={{ background: 'rgba(4,8,15,0.9)', backdropFilter: 'blur(40px)' }}>
      ...
      <div className="p-4 flex items-center justify-between shrink-0 relative">
        ...
        <div><h2 className="text-[13px] font-semibold text-white/90 flex items-center gap-2"><Users className="w-3.5 h-3.5 text-emerald-400" /> My Tasks</h2><p className="text-[10px] text-white/25 mt-0.5">{tasks.filter(t=>t.status==='active').length} active · {tasks.filter(t=>t.status==='pending').length} pending</p></div>
        ...
        <GlassCard padding="p-3.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[{ icon: Globe, label: 'Translate', c: 'text-blue-400 hover:bg-blue-500/8' }, { icon: AlertTriangle, label: 'Report', c: 'text-rose-400 hover:bg-rose-500/8' }].map(a => (
              <button key={a.label} className={cn("flex items-center gap-2 p-2.5 rounded-xl bg-white/[0.02] border border-white/[0.05] transition-colors text-[11px] font-medium", a.c)}><a.icon className="w-3.5 h-3.5" /> {a.label}</button>
            ))}
          </div>
        </GlassCard>
