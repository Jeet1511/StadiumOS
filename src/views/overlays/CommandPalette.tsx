 ...
 * @description Overlay: Keyboard-accessible command palette (Ctrl+K).
 * Provides quick access to all stadium features, AI assistant, and emergency protocols.
 *
 * Accessibility: role="dialog", aria-modal, aria-label on search input,
 * role="listbox" with role="option" for results, focus trapping.
 *
 * Hackathon Alignment: Operational intelligence — quick command access
 * for navigation, crowd management, AI assistant, multilingual support.
 */
        ...
        >
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ type: 'spring', stiffness: 350, damping: 30 }}
            ...
            <div className="p-3 flex items-center gap-3 border-b border-white/[0.06]">
              ...
              />
              <button onClick={onClose} aria-label="Close command palette" className="p-1 hover:bg-white/[0.06] rounded-lg transition-colors"><X className="w-3.5 h-3.5 text-white/30" /></button>
            </div>
